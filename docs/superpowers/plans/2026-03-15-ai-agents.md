...
src/
  ai/
    ...
    agents/
      world.ts               — World Agent: prompt + generateObject call
      event.ts               — Event Agent: prompt + generateObject call
      npc.ts                 — NPC Agent: prompt + generateObject call
      narrative.ts           — Narrative Agent: prompt + generateObject call
    orchestration/
...
- `src/types/agents.ts` — No changes needed (Plan 1 types are sufficient)
...
Run: `npm ls ai @ai-sdk/openai @ai-sdk/anthropic @ai-sdk/deepseek zod`
Expected: All 5 packages listed, no `MISSING` or `ERR`
...
Check that `.gitignore` has `.env.local`. If not, add it.
...
The Zod schemas mirror the TypeScript types in `src/types/agents.ts` and `src/types/events.ts`. They serve as runtime validation for AI structured output (via `generateObject({ schema })`).
...
// --- Agent Input Schema (for validation, not for generateObject) ---
...
2. Calls `generateObject()` with its Zod schema
3. Returns typed, validated output

Tests mock `generateObject` to avoid real API calls.
...
describe('runWorldAgent', () => {
  it('returns structured world output', async () => {
    ...
    mockedGenerateObject.mockResolvedValueOnce({ object: mockOutput } as any)
...
describe('runNPCAgent', () => {
  ...
  it('includes NPC profiles in prompt', async () => {
    mockedGenerateObject.mockResolvedValueOnce({
      object: { npcActions: [], chatMessages: [] },
    } as any)
...
function buildSystemPrompt(input: AgentInput, isCriticalPeriod: boolean): string {
...
export async function runNarrativeAgent(
  ...
  const { object } = await generateObject({
    ...
    schema: NarrativeAgentOutputSchema,
    system: buildSystemPrompt(input, isCriticalPeriod),
    prompt: buildUserPrompt(input, worldContext, eventContext, npcContext, playerActions),
  })
...
describe('validateChoices', () => {
  it('filters choices exceeding remaining stamina', () => {
    ...
    const result = validateChoices(choices, 2, 'onboarding' as CriticalPeriodType)
  ...
  it('filters choices with invalid category for the critical period type', () => {
    ...
    const result = validateChoices(choices, 3, 'onboarding' as CriticalPeriodType)
    expect(result).toHaveLength(1)
    expect(result[0].category).toBe('学习')
  })

  it('clamps stat changes in effects to valid range', () => {
    const choices: CriticalChoice[] = [
      {
    ...
    const result = validateChoices(choices, 3, 'onboarding' as CriticalPeriodType)
    expect(result[0].effects.statChanges!.professional).toBeLessThanOrEqual(10)
  })

  it('keeps at least 1 valid choice', () => {
    const choices: CriticalChoice[] = [
      { choiceId: 'a', label: 'A', staminaCost: 5, effects: {}, category: '学习' },
      { choiceId: 'b', label: 'B', staminaCost: 5, effects: {}, category: '社交' },
    ]
    // All exceed stamina — but we need at least 1
    const result = validateChoices(choices, 1, 'onboarding' as CriticalPeriodType)
...
const MAX_STAT_CHANGE_PER_CHOICE = 10
 ...
 * - stat changes clamped to reasonable range
...
export function validateChoices(
  ...
  // Clamp stat changes in valid choices
  valid = valid.map(choice => ({
    ...choice,
    effects: {
      ...choice.effects,
      statChanges: choice.effects.statChanges
        ? Object.fromEntries(
            Object.entries(choice.effects.statChanges).map(([key, val]) => [
              key,
              Math.max(-MAX_STAT_CHANGE_PER_CHOICE, Math.min(MAX_STAT_CHANGE_PER_CHOICE, val as number)),
            ]),
          )
        : undefined,
    },
  ...
  // Guarantee at least 1 choice (adjust stamina cost of first original if needed)
...
export async function runQuarterlyPipeline(
  ...
  const recentHistory = getRecentHistory(state.history)
...
export async function runCriticalDayPipeline(
  ...
  const narrativeOutput = await runNarrativeAgent(
    { state: settledState, recentHistory },
...
This is the main API endpoint that the frontend calls. It receives the player's action (quarterly plan or critical choice), runs the full AI pipeline, and returns the result. It replaces the simple engine-only `action` route for AI-powered gameplay.
...
git commit -m "feat: add /api/game/turn route with AI pipeline dispatch"
```
